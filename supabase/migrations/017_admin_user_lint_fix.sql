...
begin
  select om.university_id into admin_university
  from public.organization_memberships om
  where om.user_id=auth.uid() and om.role='university_admin'
  limit 1;
  if admin_university is null then raise exception 'Not authorized'; end if;
  return query
  select p.id,coalesce(p.display_name,''),coalesce(u.email,''),r.name,f.name,
    coalesce(sum(ge.xp_delta),0)::bigint,
    public.current_level(coalesce(sum(ge.xp_delta),0)),
  ...
  join public.residences r on r.id=sm.residence_id
  join public.floors f on f.id=sm.floor_id
  left join public.game_events ge on ge.user_id=p.id
  where sm.university_id=admin_university
    and (search_text='' or p.display_name ilike '%'||search_text||'%' or u.email ilike '%'||search_text||'%')
  group by p.id,u.email,r.name,f.name
